...
def main() -> None:
    ...
    out_path = RESULTS_DIR / "scorecard_query_transform.md"
    out_path.write_text(md, encoding="utf-8")
    print(f"\nWrote query-transform scorecard: {out_path}")

    src_log = LOGS_DIR / f"grading_run_{config['label']}.json"
    if src_log.exists():
        dst_log = LOGS_DIR / "grading_run_query_transform.json"
        dst_log.write_text(src_log.read_text(encoding="utf-8"), encoding="utf-8")
        print(f"Wrote query-transform grading log copy: {dst_log}")
