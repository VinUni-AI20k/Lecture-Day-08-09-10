...
def preprocess_document(raw_text: str, filepath: str) -> Dict[str, Any]:
    ...
    meta_patterns = {
        ...
        "access": re.compile(r"^Access:\s*(.*)$", re.I),
    }
    ...
    for line in lines:
        if not header_done:
            ...
            if line.startswith("===") or line.strip() == "" or (line.isupper() and len(line) > 5):
                if line.startswith("==="):
                    header_done = True
...
def _split_by_size(
    ...
    for para in paragraphs:
        if len(current_chunk_text) + len(para) <= chunk_chars:
            ...
            # Khởi tạo chunk mới với overlap từ đoạn cuối của chunk vừa rồi
...
def build_index(docs_dir: Path = DOCS_DIR, db_dir: Path = CHROMA_DB_DIR) -> None:
    ...
    for filepath in doc_files:
        ...
        embeddings = []
        # Chunking the batch to avoid rate limits if necessary, but for lab size it's fine
