...
# Variant: chỉ hybrid (dense + BM25 + RRF), không rerank — so với baseline dense (cùng top_k)
VARIANT_CONFIG = {
    "retrieval_mode": "hybrid",
    "top_k_search": 10,
    "top_k_select": 3,
    "use_rerank": False,
    # Chỉ sửa ở eval: nếu hybrid retrieve hụt source kỳ vọng thì fallback dense
    "fallback_to_dense_on_low_recall": True,
    "fallback_min_recall": 1.0,
...
def score_faithfulness(
    ...
    abstain = any(
        p in low
        for p in ("không đủ dữ liệu", "không đủ thông tin", "insufficient", "do not know")
    )
    if abstain and not chunks_used:
        return {"score": 5, "notes": "Abstain khi không có context — phù hợp grounded"}
    if abstain and chunks_used:
        return {"score": 4, "notes": "Abstain dù có context — kiểm tra lại prompt/gate"}

    if not chunks_used:
        return {"score": 2, "notes": "Không có chunk để neo câu trả lời"}

    # Lấy max overlap theo từng chunk (tốt hơn ghép cả khối dài làm loãng điểm)
    overlaps = [_text_sim(answer, c.get("text", "") or "") for c in chunks_used]
    overlap = max(overlaps) if overlaps else 0.0
    score = max(1, min(5, int(round(overlap * 4)) + 1))
    return {
        "score": score,
        "notes": f"Heuristic max(answer↔chunk) ≈ {overlap:.2f}",
    }
...
def score_answer_relevance(
    ...
    """
    rel = _text_sim(query, answer)
    score = max(1, min(5, int(round(rel * 4)) + 1))
    return {
        "score": score,
        "notes": f"Heuristic query↔answer similarity ≈ {rel:.2f}",
    }
...
def score_completeness(
    ...
    sim = _text_sim(answer, expected_answer)
    score = max(1, min(5, int(round(sim * 4)) + 1))
    return {
        "score": score,
        "notes": f"Heuristic answer↔expected ≈ {sim:.2f}",
    }
...
def run_scorecard(
    ...
    for q in test_questions:
        ...
        selected_mode = config.get("retrieval_mode", "dense")
        ...
        try:
            ...
            # Nếu variant hybrid bị hụt retrieval thì fallback sang dense cho câu hiện tại.
            should_fallback = (
                selected_mode == "hybrid"
                and config.get("fallback_to_dense_on_low_recall", False)
                and bool(expected_sources)
            )
            if should_fallback:
                hybrid_recall = score_context_recall(chunks_used, expected_sources).get("recall")
                min_recall = float(config.get("fallback_min_recall", 1.0))
                if hybrid_recall is None:
                    hybrid_recall = 0.0

                if hybrid_recall < min_recall:
                    dense_result = rag_answer(
                        query=query,
                        retrieval_mode="dense",
                        top_k_search=config.get("top_k_search", 10),
                        top_k_select=config.get("top_k_select", 3),
                    ...
                    dense_chunks = dense_result.get("chunks_used", [])
                    dense_recall = score_context_recall(dense_chunks, expected_sources).get("recall")
                    dense_recall = 0.0 if dense_recall is None else dense_recall

                    if dense_recall > hybrid_recall:
                        answer = dense_result["answer"]
                        chunks_used = dense_chunks
                        selected_mode = "dense_fallback"
...
if __name__ == "__main__":
    ...
    print("\n--- Chạy Baseline ---")
    print("Lưu ý: Cần OPENAI_API_KEY hoặc Gemini trong .env để chấm generation.")
    try:
        ...
        print(f"Lỗi khi chạy baseline scorecard: {e}")
    ...
    try:
        ...
        print(f"Lỗi khi chạy variant scorecard: {e}")
    ...
    print("\nĐã chạy xong eval. Kết quả trong thư mục results/")
