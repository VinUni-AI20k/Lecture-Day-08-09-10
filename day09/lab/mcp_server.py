...
# ─────────────────────────────────────────────
# Real MCP HTTP Server (FastAPI)
# ─────────────────────────────────────────────

if HAS_FASTAPI:
    app = FastAPI(title="Day 09 Mock MCP Server")

    @app.get("/tools")
    async def api_list_tools():
        """Endpoint tương ứng với list_tools()"""
        return {"tools": list_tools()}

    @app.post("/call")
    async def api_dispatch_tool(request: Request):
        """Endpoint tương ứng với dispatch_tool()"""
        try:
...
if __name__ == "__main__":
    ...
    if "--server" in sys.argv:
        ...
        print("[SERVER] Starting MCP HTTP Server at http://localhost:8000")
        print("   - GET  /tools : List available tools")
        print("   - POST /call  : Call a tool")
        uvicorn.run(app, host="localhost", port=8000)
    else:
        print("=" * 60)
        print("MCP Server - Tool Discovery & Test")
        print("=" * 60)
        print("[HINT] Run 'python mcp_server.py --server' to start Real MCP Server.")
